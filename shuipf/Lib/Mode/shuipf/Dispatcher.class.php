...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        $App = F("App");
        // 开启子域名部署
        if (isset($App['Domains']) && $App['Domains']) {
            ...
            if (!$Domains_cache) {
                try {
                    ...
                    // 缓存不存在 抛出异常
                    throw_exception('缓存 "Domains_list" 加载失败！ ');
                }
            }
            //子域名部署规则
            $rules = array();
            foreach ($Domains_cache as $domain => $rul) {
                $rules[$domain] = array(
                    $rul . "/",
                );
            }
            C('APP_SUB_DOMAIN_RULES', $rules);
        }
        ...
        if (C('APP_SUB_DOMAIN_DEPLOY')) {
            ...
            if (!empty($rule)) {
                //分配当前绑定域名
                C("APP_SUB_DOMAIN", strtolower($_SERVER['HTTP_HOST']));
                // 子域名部署规则 '子域名'=>array('分组名/[模块名]','var1=a&var2=b');
                $array = explode('/', $rule[0]);
                $module = array_pop($array);
                if (!empty($module)) {
                    C("DEFAULT_MODULE", $module);
                    $domainModule = true;
                }
                if (!empty($array)) {
                    C("DEFAULT_GROUP", array_pop($array));
                    $domainGroup = true;
                }
