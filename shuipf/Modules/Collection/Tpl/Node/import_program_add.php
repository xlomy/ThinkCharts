...
<div class="wrap J_check_wrap">
  ...
  <form name="myform" action="{:U('Node/import_program_add', array('nodeid'=>$nodeid,'type'=>$type,'ids'=>$ids,'catid'=>$catid) )}" method="post" >
    <div class="table_full">
      <table width="100%">
        <tr>
          <th width="120">方案名称：</th>
          <td> <input type="text" name="name" value=" {$cat['catname']} 的发布方案"   class="input"> </td>
        </tr>
        <tr>
          <th width="120">栏目：</th>
          <td> {$cat['catname']} </td>
        ...
        <tr>
          ...
          <td><label>
              ...
              是否截取内容</label>
            <input type="text" name="introcude_length" value="200" size="3" class="input">
            字符至内容摘要 </td>
        </tr>
        <tr>
          <th width="120">自动提取缩略图：</th>
          <td><label><input type='checkbox' name='auto_thumb' value="1">是否获取内容第</label>
            <input type="text" name="auto_thumb_no" value="1" size="2" class="input">张图片作为标题图片 </td>
        </tr>
        <tr>
          <th width="120">导入文章状态：</th>
          <td><label><input type="radio" name="content_status" id="_99" checked value="99"> 审核通过</label> <label><input type="radio" name="content_status" id="_1" value="1"> 待审核</label> </td>
        </tr>
      </table>
    </div>
    <div class="bk10"></div>
...
?>
          <tr>
            <td>{$v.field}</td>
            <td>{$v.name}</td>
            <td><input type="hidden" name="model_field[]" value="<?php echo $v['field']?>"><?php echo Form::select($node_field, (in_array($v['field'], array('inputtime', 'updatetime')) ? 'time' : $v['field']), 'name="node_field[]"')?></td>
