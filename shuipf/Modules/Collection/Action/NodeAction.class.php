...
class NodeAction extends AdminbaseAction {
    ...
    public function import_content() {
        ...
        foreach ($data as $k => $v) {
            ...
            foreach ($program['config']['default'] as $k => $b) {
                //默认值填充
                if (empty($v['data'][$k]) && isset($program['config']['default'][$k])) {
            ...
            if ($contentid) {
                ...
                $i++;
                //更新附件
                $aids = M("Attachment_index")->where(array('keyid' => 'cj-' . $nodeid . '-' . $v['id']))->field("aid")->select();
                if ($aids) {
                    $a_ids = array();
                    foreach ($aids as $r) {
                        $a_ids[] = $r['aid'];
    ...
    public function import_program_add() {
        ...
        if (IS_POST) {
            ...
            //读取数据模型缓存
            $model = F('Model_field_' . $cat['modelid']);
            if (empty($model)) {
                $this->error("模型不存在！");
            }
            $node_data = $info;
